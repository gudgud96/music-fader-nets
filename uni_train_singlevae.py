...
def train(step, d_oh, r_oh, n_oh,
    ...
    output = loss.item(), CE_X.item(), l_r.item(), l_n.item(), 0, 0
    return step, output
    ...
    for i in range(1, args['n_epochs'] + 1):
        ...
        for j, x in tqdm(enumerate(train_dl_dist), total=len(train_dl_dist)):
            ...
            step, loss = train(step, d_oh, r_oh, n_oh,
                                            d, r, n, c, c_r, c_n, c_r_oh, c_n_oh, r_density, n_density)
            loss, CE_X, l_r, l_n, l_adv_r, l_adv_n = loss
            batch_loss += loss
...
def evaluation_phase():
    ...
    def run(dl):
        ...
        for i, x in tqdm(enumerate(dl), total=len(dl)):
            ...
            reg = LinearRegression().fit(z[:, 0].unsqueeze(-1).cpu().detach().numpy(), 
                                        r_density.unsqueeze(-1).cpu().detach().numpy())
            linear_r.append(reg.score(z[:, 0].unsqueeze(-1).cpu().detach().numpy(), 
                    r_density.unsqueeze(-1).cpu().detach().numpy()))

            reg = LinearRegression().fit(z[:, 1].unsqueeze(-1).cpu().detach().numpy(), 
                                        n_density.unsqueeze(-1).cpu().detach().numpy())
            linear_n.append(reg.score(z[:, 1].unsqueeze(-1).cpu().detach().numpy(), 
                    n_density.unsqueeze(-1).cpu().detach().numpy()))
