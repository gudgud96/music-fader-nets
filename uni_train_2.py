...
def latent_regularized_loss_function_v1(z_out, r, n):
    ...
    # print(torch.tanh(D_z_r[0]))
    # print(torch.sign(D_attr_r[0]))
    ...
    # print()
    # print(torch.tanh(D_z_n[0]))
    # print(torch.sign(D_attr_n[0]))
...
def train(step, d_oh, r_oh, n_oh,
    ...
    loss, CE_X, CE_R, CE_N = loss_function(out, d,
                                        ...
                                        beta=args['beta'])
    if is_class:
        cls_loss = class_loss_function(cls_r, c_r, cls_n, c_n)
        loss += cls_loss
...
def evaluation_phase():
    ...
    def run(dl):
        ...
        for i, x in tqdm(enumerate(dl), total=len(dl)):
            ...
            # for n_sample in n:
            #     n_density = np.log(sum(n_sample.cpu().detach().numpy()) / len(n_sample))
            #     n_density_lst.append(n_density)
            
            # r_density_lst, n_density_lst = r_density_lst.cpu().detach().numpy(), n_density_lst.cpu().detach().numpy()
            ...
            # class accuracy update
            def class_acc(output, target):
                acc = 0 
                for idx in range(len(target)):
                    if torch.argmax(output[idx], dim=-1).item() == target[idx].item():
                        acc += 1
                return acc
            
            if is_class:
                acc_c_r, acc_c_n = class_acc(cls_r, c_r), class_acc(cls_n, c_n)
                inputs = c_acc_r, c_acc_n
                updates = acc_c_r, acc_c_n
                outputs = []
                for idx in range(len(inputs)):
                    outputs.append(inputs[idx] + updates[idx])
                c_acc_r, c_acc_n = outputs
