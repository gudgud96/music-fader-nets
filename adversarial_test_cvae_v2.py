...
class NoteCVAEEvaluator(CVAEEvaluator):
    def __init__(self, ds, epochs=10, num_of_samples=100):
        super().init(ds, epochs=epochs, num_of_samples=num_of_samples)
...
if __name__ == "__main__":
    ...
    # rhythm_evaluator = RhythmCVAEEvaluator(test_ds_dist)
    rhythm_evaluator = RhythmFaderNetsEvaluator(test_ds_dist)
    rhythm_evaluator.evaluate(model)

    print("Note Generation")
    # note_evaluator = NoteCVAEEvaluator(test_ds_dist)
    note_evaluator = NoteFaderNetsEvaluator(test_ds_dist)
