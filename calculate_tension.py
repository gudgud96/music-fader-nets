...
def key_search(ce, shift,key_given=False,is_minor=False):
    ...
    major_diff_lst = [0 for _ in range(12)]
    minor_diff_lst = [0 for _ in range(12)]
    major_keypos_lst = []
    minor_keypos_lst = []

    # store initial key
    if not is_minor:
        initial_key_pos = major_key_position(note_index_to_pitch_index[shift][shift])
    else:
        minor_shift = shift - 3
        if minor_shift < 0:
            minor_shift += 12
        initial_key_pos = minor_key_position(note_index_to_pitch_index[shift][minor_shift])
    
    # if it is a given key, just use as is
    if key_given:
        if not is_minor:
            ...
            if shift - 3 in sharp_index:
                result = f'minor {pitch_index_to_sharp_names[shift - 3]}'
                # print(result)
                shift = shift - 3
            else:
                result = f'minor {pitch_index_to_flat_names[shift - 3]}'
                # print(result)
                shift = shift - 3
        return initial_key_pos, result, shift

    # traverse through all major and minor keys to find the least distance
    for i in range(12):
        major_key_pos = major_key_position(note_index_to_pitch_index[i][i])

        minor_shift = i - 3
        if minor_shift < 0:
            minor_shift += 12

        minor_key_pos = minor_key_position(note_index_to_pitch_index[i][minor_shift])

        diff_major = np.linalg.norm(ce - major_key_pos)
        diff_minor = np.linalg.norm(ce - minor_key_pos)
        major_diff_lst[i], minor_diff_lst[i] = diff_major, diff_minor
        major_keypos_lst.append(major_key_pos)
        minor_keypos_lst.append(minor_key_pos)
    
    # find closest key position as estimation
    min_major_idx, min_major_val = major_diff_lst.index(min(major_diff_lst)), min(major_diff_lst)
    min_minor_idx, min_minor_val = minor_diff_lst.index(min(minor_diff_lst)), min(minor_diff_lst)
    if min_major_val < min_minor_val:
        if min_major_idx in sharp_index:
            result = f'{pitch_index_to_sharp_names[min_major_idx]} major'
            # print(result)
        else:
            result = f'{pitch_index_to_flat_names[min_major_idx]} major'
            # print(result)
        return major_keypos_lst[min_major_idx], result, min_major_idx
    
    else:
        if min_minor_idx in sharp_index:
            result = f'{pitch_index_to_sharp_names[min_minor_idx]} minor'
            # print(result)
        else:
            result = f'{pitch_index_to_flat_names[min_minor_idx]} minor'
            # print(result)
        return minor_keypos_lst[min_minor_idx], result, min_minor_idx
...
def calculate_diameter(file_name, pm, beats, output_folder, 
                        window_size=1, key_index=None, is_minor=False):
    
    base_name = os.path.basename(file_name)

    if key_index is None:
        key_index, is_minor = get_key_index_v2(pm)
        key_given = False
    else:
        key_given = True
    ...
    key_pos, key_name, new_key_idx = cal_key(chord_roll_eighth, key_index, key_given, is_minor)

    # assume no key change at the moment
    key_change_bar = -1
    diameters = diameter(chord, key_index, key_change_bar, new_key_idx)
    diameters = np.array(diameters)
    # diameters = merge_tension(diameters,window_size)
    
    return diameters, np.mean(np.trim_zeros(diameters))
...
def get_key_index(pm):
    ...
    if frequent in diatonic_scales:
        print(diatonic_scales.index(frequent), "index")
        return diatonic_scales.index(frequent),False
    if frequent in harmonic_scales:
        print(harmonic_scales.index(frequent), "index")
...
def get_key_index_v2(pm):
    pm.write("key_index_tmp.mid")
    ...
    score = music21.converter.parse("key_index_tmp.mid")
    key = score.analyze('key')
    res = np.zeros(24,)
    name, mode = key.tonic.name, key.mode
    # print(name, mode)

    return CHORD_DICT[name], mode == "minor"
...
def get_piano_roll(pm, beat_times=None):
...
if __name__== "__main__":
    from ptb_v2 import slice_midi

    labelled_midi = ["/data/piano-e-competition/" + k for k in os.listdir("/data/piano-e-competition/")]
    name = labelled_midi[0]
    pm = pretty_midi.PrettyMIDI(name)
    beats = pm.get_beats()
    new_pm = slice_midi(pm, beats, 32, 64)
    key_index, is_minor = get_key_index(new_pm)
    print(key_index, is_minor)
    pr = get_piano_roll(new_pm)
    
    print(get_centroid(pr, key_index))
