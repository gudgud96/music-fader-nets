...
class MusicAttrRegHVAE(nn.Module):
    def __init__(self,
                 roll_dims,
                 rhythm_dims,
                 note_dims,
                 ...
                 n_step,
                 k=1000):
        super(MusicAttrRegHVAE, self).__init__()
        ...
        # inference mu and logvar
        self.mu_r, self.var_r = nn.Linear(hidden_dims * 2, z_dims), nn.Linear(hidden_dims * 2, z_dims)
        self.mu_n, self.var_n = nn.Linear(hidden_dims * 2, z_dims), nn.Linear(hidden_dims * 2, z_dims)
        self.mu_c, self.var_c = nn.Linear(hidden_dims * 2, z_dims), nn.Linear(hidden_dims * 2, z_dims)

        # inference high mu and logvar
        self.mu_high, self.var_high = nn.Linear(z_dims * 2, z_dims), nn.Linear(z_dims * 2, z_dims)

        # dropouts
        self.e_dropout = nn.Dropout(p=0.3)

        # generation mu and logvar
        self.mu_r_g, self.var_r_g = nn.Linear(z_dims, z_dims), nn.Linear(z_dims, z_dims)
        self.mu_n_g, self.var_n_g = nn.Linear(z_dims, z_dims), nn.Linear(z_dims, z_dims)
        self.mu_c_g, self.var_c_g = nn.Linear(z_dims, z_dims), nn.Linear(z_dims, z_dims)
    ...
    def forward(self, x, rhythm, note, chroma, c_r_oh, c_n_oh,
        ...
        # hierachical encoder
        z_concat = torch.cat([z_r, z_n], dim=-1)
        mu_z_h, var_z_h = self.mu_high(z_concat), self.var_high(z_concat).exp_()
        dis_h = Normal(mu_z_h, var_z_h)

        z_h = repar(dis_h.mean, dis_h.stddev)

        # decoding
        mu_r_g, var_r_g = self.mu_r_g(z_h), self.var_r_g(z_h).exp_()
        mu_n_g, var_n_g = self.mu_n_g(z_h), self.var_n_g(z_h).exp_()

        dis_r_g = Normal(mu_r_g, var_r_g)
        dis_n_g = Normal(mu_n_g, var_n_g)

        z_r_g = repar(dis_r_g.mean, dis_r_g.stddev)
        z_n_g = repar(dis_n_g.mean, dis_n_g.stddev)

        # get sub decoders output
        r_out, n_out, r_density, n_density = self.sub_decoders(rhythm, z_r_g, note, z_n_g)

        # packaging output
        z = torch.cat([z_r_g, z_n_g, chroma], dim=1)        
        out = self.global_decoder(z, steps=x.shape[1])
        output = (out, r_out, n_out, r_density, n_density)
        dis = (dis_r, dis_n, dis_h, dis_r_g, dis_n_g)
        z_out = (z_r, z_n, z_h, z_r_g, z_n_g)
