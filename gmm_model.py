...
class MusicAttrRegGMVAE(nn.Module):
    ...
    def _approx_qy_x(self, z, mu_lookup, logvar_lookup, n_component):
        """
    ...
    def forward(self, x, rhythm, note, chroma, c_r_oh, c_n_oh,
        ...
        logLogit_qy_x_r, qy_x_r = self._approx_qy_x(z_r, self.mu_r_lookup, self.logvar_r_lookup, n_component=self.n_component)
        _, y_r = torch.max(qy_x_r, dim=1)

        logLogit_qy_x_n, qy_x_n = self._approx_qy_x(z_n, self.mu_n_lookup, self.logvar_n_lookup, n_component=self.n_component)
