...
if __name__ == "__main__":
    ...
    attribute_loss = torch.nn.BCELoss()
    ...
    attr_r = Classifier(latent_dim=args["z_dim"])
    attr_n = Classifier(latent_dim=args["z_dim"])
    ...
    def pass_model(x, is_supervised=True):
    ...
    for epoch in range(args["n_epochs"]):
        ...
        for i, x in enumerate(vgm_train_dl_dist):
            ...
            attribute_r, attribute_n = attr_r(z_r_p_gen), attr_n(z_n_p_gen)
            validity_int_r, validity_int_n = validity_int(validity_r), validity_int(validity_n)
            attribute_int_r, attribute_int_n = validity_int(attribute_r), validity_int(attribute_n)
            
            g_loss = adversarial_loss(validity_r, valid) + adversarial_loss(validity_n, valid)

            a_loss = attribute_loss(attribute_r, torch.argmax(a_label, dim=-1).unsqueeze(-1).float()) + \
                    attribute_loss(attribute_r, torch.argmax(a_label, dim=-1).unsqueeze(-1).float())
            g_acc = (accuracy_score(np.ones(valid.shape[0]), validity_int_r) + \
                    accuracy_score(np.ones(valid.shape[0]), validity_int_n)) / 2
            g_a_acc = (accuracy_score(torch.argmax(a_label, dim=-1).cpu().detach().numpy().squeeze(), attribute_int_r.squeeze()) + \
                        accuracy_score(torch.argmax(a_label, dim=-1).cpu().detach().numpy().squeeze(), attribute_int_n.squeeze())) / 2

            total_loss = g_loss + a_loss
            total_loss.backward()
            optimizer_Gr.step()
            optimizer_Gn.step()
            ...
            print(
                "[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f] [A loss: %f] [D acc: %f] [G acc: %f] [A acc: %f]]"
                % (epoch, args["n_epochs"], i, len(vgm_train_dl_dist), d_loss.item(), g_loss.item(), a_loss.item(), d_acc, g_acc, g_a_acc)
            )
        ...
        for i, x in enumerate(train_dl_dist):
            ...
            z_r_q, z_n_q, _ = pass_model(x, is_supervised=False)
            ...
            # Generate a batch of images
            valid_oh = torch.Tensor(np.eye(2, dtype='uint8')[valid.int().cpu().detach().numpy()]).cuda().squeeze()
            z_r_p_gen, z_n_p_gen = generator_r(z_r_p, valid_oh), generator_n(z_n_p, valid_oh)

            # Loss measures generator's ability to fool the discriminator
            validity_r, validity_n = discriminator_r(z_r_p_gen, valid_oh), discriminator_n(z_n_p_gen, valid_oh)
            attribute_r, attribute_n = attr_r(z_r_p_gen), attr_n(z_n_p_gen)
            validity_int_r, validity_int_n = validity_int(validity_r), validity_int(validity_n)
            attribute_int_r, attribute_int_n = validity_int(attribute_r), validity_int(attribute_n)
            
            g_loss_1 = adversarial_loss(validity_r, valid) + adversarial_loss(validity_n, valid)
            a_loss_1 = attribute_loss(attribute_r, valid) + attribute_loss(attribute_r, valid)
            g_acc_1 = (accuracy_score(np.ones(valid.shape[0]), validity_int_r) + \
                    accuracy_score(np.ones(valid.shape[0]), validity_int_n)) / 2
            g_a_acc_1 = (accuracy_score(valid.cpu().detach().numpy().squeeze(), attribute_int_r) + \
                        accuracy_score(valid.cpu().detach().numpy().squeeze(), attribute_int_n)) / 2

            total_loss = g_loss_1 + a_loss_1
            total_loss.backward()
            optimizer_Gr.step()
            optimizer_Gn.step()
            ...
            # Generate another batch of images
            fake_oh = torch.Tensor(np.eye(2, dtype='uint8')[fake.int().cpu().detach().numpy()]).cuda().squeeze()
            z_r_p_gen, z_n_p_gen = generator_r(z_r_p, fake_oh), generator_n(z_n_p, fake_oh)

            # Loss measures generator's ability to fool the discriminator
            validity_r, validity_n = discriminator_r(z_r_p_gen, fake_oh), discriminator_n(z_n_p_gen, fake_oh)
            attribute_r, attribute_n = attr_r(z_r_p_gen), attr_n(z_n_p_gen)
            validity_int_r, validity_int_n = validity_int(validity_r), validity_int(validity_n)
            attribute_int_r, attribute_int_n = validity_int(attribute_r), validity_int(attribute_n)
            
            g_loss_2 = adversarial_loss(validity_r, valid) + adversarial_loss(validity_n, valid)
            a_loss_2 = attribute_loss(attribute_r, fake) + attribute_loss(attribute_r, fake)
            g_acc_2 = (accuracy_score(np.ones(valid.shape[0]), validity_int_r) + \
                    accuracy_score(np.ones(valid.shape[0]), validity_int_n)) / 2
            g_a_acc_2 = (accuracy_score(fake.cpu().detach().numpy().squeeze(), attribute_int_r) + \
                        accuracy_score(fake.cpu().detach().numpy().squeeze(), attribute_int_n)) / 2

            total_loss = g_loss_2 + a_loss_2
            total_loss.backward()
            optimizer_Gr.step()
            optimizer_Gn.step()
            ...
            # Loss for q(z|x)
            validity_q_r, validity_q_n = discriminator_r(z_r_q, valid_oh), discriminator_n(z_n_q, valid_oh)
            validity_int_qr, validity_int_qn = validity_int(validity_q_r), validity_int(validity_q_n)
            d_real_acc = (accuracy_score(np.ones(valid.shape[0]), validity_int_qr) + \
                            accuracy_score(np.ones(valid.shape[0]), validity_int_qn)) / 2
            d_real_loss = adversarial_loss(validity_q_r, valid) + adversarial_loss(validity_q_n, valid)
            ...
            # Loss for p(z)
            validity_p_r, validity_p_n = discriminator_r(z_r_p, valid_oh), discriminator_n(z_n_p, valid_oh)
            validity_int_pr, validity_int_pn = validity_int(validity_p_r), validity_int(validity_p_n)
            d_fake_acc = (accuracy_score(np.zeros(valid.shape[0]), validity_int_pr) + \
                            accuracy_score(np.zeros(valid.shape[0]), validity_int_pn)) / 2
            d_fake_loss = adversarial_loss(validity_p_r, fake) + adversarial_loss(validity_p_n, fake)
            ...
            # Loss for G(p(z), y)
            validity_g_r, validity_g_n = discriminator_r(z_r_p_gen, valid_oh), discriminator_n(z_n_p_gen, valid_oh)
            validity_int_gr, validity_int_gn = validity_int(validity_g_r), validity_int(validity_g_n)
            d_gen_acc = (accuracy_score(np.zeros(valid.shape[0]), validity_int_gr) + \
                            accuracy_score(np.zeros(valid.shape[0]), validity_int_gn)) / 2
            d_gen_loss = adversarial_loss(validity_g_r.detach(), fake) + adversarial_loss(validity_g_n.detach(), fake)
            ...
            # Loss for q(z|x)
            validity_q_r, validity_q_n = discriminator_r(z_r_q, fake_oh), discriminator_n(z_n_q, fake_oh)
            validity_int_qr, validity_int_qn = validity_int(validity_q_r), validity_int(validity_q_n)
            d_real_acc = (accuracy_score(np.ones(valid.shape[0]), validity_int_qr) + \
                            accuracy_score(np.ones(valid.shape[0]), validity_int_qn)) / 2
            d_real_loss = adversarial_loss(validity_q_r, valid) + adversarial_loss(validity_q_n, valid)
            ...
            # Loss for p(z)
            validity_p_r, validity_p_n = discriminator_r(z_r_p, fake_oh), discriminator_n(z_n_p, fake_oh)
            validity_int_pr, validity_int_pn = validity_int(validity_p_r), validity_int(validity_p_n)
            d_fake_acc = (accuracy_score(np.zeros(valid.shape[0]), validity_int_pr) + \
                            accuracy_score(np.zeros(valid.shape[0]), validity_int_pn)) / 2
            d_fake_loss = adversarial_loss(validity_p_r, fake) + adversarial_loss(validity_p_n, fake)
            ...
            # Loss for G(p(z), y)
            validity_g_r, validity_g_n = discriminator_r(z_r_p_gen, fake_oh), discriminator_n(z_n_p_gen, fake_oh)
            validity_int_gr, validity_int_gn = validity_int(validity_g_r), validity_int(validity_g_n)
            d_gen_acc = (accuracy_score(np.zeros(valid.shape[0]), validity_int_gr) + \
                            accuracy_score(np.zeros(valid.shape[0]), validity_int_gn)) / 2
            d_gen_loss = adversarial_loss(validity_g_r.detach(), fake) + adversarial_loss(validity_g_n.detach(), fake)
            ...
            g_loss = (g_loss_1.item() + g_loss_2.item()) / 2
            g_acc = (g_acc_1.item() + g_acc_2.item()) / 2
            a_loss = (a_loss_1.item() + a_loss_2.item()) / 2
            a_acc = (g_a_acc_1.item() + g_a_acc_2.item()) / 2

            print(
                "[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f] [A loss: %f]  [D acc: %f] [G acc: %f] [A acc: %f]"
                % (epoch, args["n_epochs"], i, len(vgm_train_dl_dist), d_loss.item(), g_loss, a_loss, d_acc, g_acc, a_acc)
            )
